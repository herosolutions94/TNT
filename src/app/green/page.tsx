...
/* ─── DATA (Updated with Green Accents) ────────────────── */
...
const interns = [
  { img: '/images/img1.jpg', name:'Aysha', outcome: 'Landed her dream marketing role at a Series B startup in Dubai.', tag: 'Full-time Hire', color: '#00FF66' },
  { img: '/images/img2.jpg', name:'Mubashra', outcome: 'Earned $18,000 in a single month from performance bonuses.', tag: '$18K Month', color: '#00cc52' },
  { img: '/images/img3.jpg', name:'Johen', outcome: 'Grew from 0 content experience to 400K followers in 90 days.', tag: '400K Followers', color: '#00fa9a' },
]
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen noise" style={{ background: '#0a0a0a' }}>
      <NavbarGreen />

      {/* ── HERO ── */}
      <section className="relative flex flex-col items-center justify-center text-center px-6 pt-36 pb-24 overflow-hidden min-h-screen bg-[#050505]">
        ...
        <div className="absolute inset-0 z-0 pointer-events-none">
          ...
          <div className="absolute inset-0 opacity-[0.03]" 
            style={{ backgroundImage: `linear-gradient(#00FF66 1px, transparent 1px), linear-gradient(90deg, #00FF66 1px, transparent 1px)`, backgroundSize: '40px 40px' }} />
        </div>
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          <ScrollReveal>
            <span className="inline-block px-4 py-1.5 rounded-full text-xs font-semibold tracking-widest uppercase mb-6"
              style={{ background: 'rgba(0,255,102,0.1)', color: '#00FF66', border: '1px solid rgba(0,255,102,0.3)' }}>
              Hiring Marketing Interns
            </span>

            <h1 className="font-display font-bold leading-tight mb-6"
              style={{ fontSize: 'clamp(3.5rem, 8vw, 7.5rem)', color: '#F5F0E8' }}>
              Get Paid to<br />
              <span className="animate-pulse" style={{ color: '#00FF66' }}>Go Viral.</span>
            </h1>
            ...
            <p className="max-w-xl mx-auto text-lg mb-10" style={{ color: 'rgba(245,240,232,0.65)', lineHeight: 1.7 }}>
              ...
              <span style={{ color: '#00FF66', fontWeight: 600 }}>$2,000 per video</span>, and learn virality from influencers with millions of followers.
            </p>

            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/apply"
                className="px-10 py-4 rounded-full text-base font-bold transition-all duration-300 hover:scale-105 hover:shadow-[0_0_40px_rgba(0,255,102,0.3)]"
              ...
              <a href="#the-job"
                ...
                style={{ background: 'rgba(245,240,232,0.06)', color: '#F5F0E8', border: '1px solid rgba(245,240,232,0.12)' }}>
                Learn More
              </a>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-10 mt-24 pt-10"
              ...
              {[['500M+', 'Views Generated'], ['$20K', 'Top Monthly Earn'], ['300+', 'Active Creators']].map(([num, label]) => (
                <div key={label} className="text-center group cursor-default">
                  <p className="font-display font-bold text-4xl mb-1 transition-transform group-hover:-translate-y-1" style={{ color: '#00FF66' }}>{num}</p>
      ...
      {/* ── TICKER ── */}
      <div className="py-4 overflow-hidden" style={{ background: '#00FF66' }}>
        <div className="ticker-wrapper">
          <div className="ticker-inner">
            {Array(8).fill('TNT CREATOR PROGRAM · GET PAID · GO VIRAL · LEARN FROM INFLUENCERS · ').join('').split('·').map((t, i) => (
              <span key={i} className="font-display font-bold text-sm tracking-widest uppercase px-6" style={{ color: '#000' }}>
      ...
      <section id="the-job" className="py-28 px-6 md:px-16 max-w-7xl mx-auto">
        <ScrollReveal>
          <p className="text-xs font-semibold tracking-widest uppercase mb-4" style={{ color: '#00FF66' }}>The Opportunity</p>
          <h2 className="font-display font-black text-5xl md:text-6xl mb-16" style={{ color: '#F5F0E8' }}>
            The Job
          </h2>
        </ScrollReveal>
          ...
          {jobPerks.map((p, i) => (
            <ScrollReveal key={p.title} delay={i * 150}>
              <div className="card-glow rounded-2xl p-8 h-full"
                ...
                <span className="text-4xl mb-6 block filter-green">
                   {/* If these icons are SVGs or PNGs, consider using a CSS filter to tint them green or replacing them */}
                  <img src={p.icon} alt='TNT' width={50} height={50} />
                </span>
                <h3 className="font-display font-bold text-xl mb-3" style={{ color: '#F5F0E8' }}>{p.title}</h3>
      ...
      {/* ── OUR INTERNS (Updated to Dark Mode with Green) ── */}
      <section id="interns" className="py-28 px-6 md:px-16" style={{ background: '#050505' }}>
        <div className="max-w-7xl mx-auto">
          <ScrollReveal>
            <p className="text-xs font-semibold tracking-widest uppercase mb-4" style={{ color: '#00FF66' }}>Real Results</p>
            <h2 className="font-display font-black text-5xl md:text-6xl mb-4" style={{ color: '#F5F0E8' }}>
              Our Interns
            </h2>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {interns.map((intern, i) => (
              <ScrollReveal key={intern.name} delay={i * 150}>
                <div className="relative overflow-hidden rounded-3xl h-[600px] group shadow-2xl" style={{ border: '1px solid rgba(0, 255, 102, 0.1)' }}>
                  <img 
                    src={intern.img} 
                    alt={intern.name} 
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-8 flex flex-col gap-3">
                    <span className="inline-block px-3 py-1 rounded-full text-xs font-bold self-start"
                      style={{ background: intern.color, color: '#000' }}>
                      {intern.tag}
                    </span>
      ...
      {/* ── WHY TNT (Dark Tech Variant) ── */}
      <section id="why-tnt" className="py-28 px-6 md:px-16" style={{ background: '#0a0a0a' }}>
        <div className="max-w-7xl mx-auto">
          <ScrollReveal>
            <p className="text-xs font-semibold tracking-widest uppercase mb-4" style={{ color: '#00FF66' }}>Perks</p>
            <h2 className="font-display font-black text-5xl md:text-6xl mb-16" style={{ color: '#F5F0E8' }}>
              Why TNT?
            </h2>
          </ScrollReveal>
            ...
            {whyItems.map((item, i) => (
              <ScrollReveal key={item.title} delay={i * 100}>
                <div className="card-glow flex gap-5 rounded-2xl p-6 h-full"
                  style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(0,255,102,0.1)' }}>
                  <span className="text-3xl shrink-0">
                    <img src={item.emoji} alt={item.title} width={40} height={40} />
                  </span>
                  <div>
                    <h3 className="font-semibold text-base mb-1.5" style={{ color: '#F5F0E8' }}>{item.title}</h3>
                    <p className="text-sm leading-relaxed" style={{ color: 'rgba(245, 240, 232, 0.5)' }}>{item.desc}</p>
                  </div>
                </div>
              </ScrollReveal>
            ))}
      ...
      {/* ── PRODUCT ── */}
      <section id="product" className="py-28 px-6 md:px-16" style={{ background: '#050505' }}>
        <div className="max-w-7xl mx-auto">
          <ScrollReveal>
            <p className="text-xs font-semibold tracking-widest uppercase mb-4" style={{ color: '#00FF66' }}>What We Build</p>
            <h2 className="font-display font-black text-5xl md:text-6xl mb-4" style={{ color: '#F5F0E8' }}>
            ...
            {productPoints.map((p, i) => (
              <ScrollReveal key={p.n} delay={i * 120}>
                <div className="card-glow rounded-2xl p-8"
                  style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(0,255,102,0.1)' }}>
                  <span className="font-display font-black text-5xl mb-4 block" style={{ color: 'rgba(0,255,102,0.15)' }}>{p.n}</span>
                  <h3 className="font-display font-bold text-xl mb-3" style={{ color: '#F5F0E8' }}>{p.title}</h3>
      ...
      {/* ── COACHES ── */}
      <section id="coaches" className="py-28" style={{ background: '#0a0a0a' }}>
          <div className="px-6 md:px-16 max-w-7xl mx-auto mb-14 text-center">
            <ScrollReveal>
              <p className="text-xs font-semibold tracking-widest uppercase mb-4" style={{ color: '#00FF66' }}>Coaching</p>
              <h2 className="font-display font-black text-5xl md:text-6xl mb-4" style={{ color: '#F5F0E8' }}>
                Content Coaches
              </h2>
            </ScrollReveal>
              ...
              {[...coaches, ...coaches].map((c, i) => (
                <div key={i} className="shrink-0 w-52 rounded-2xl p-6 text-center"
                  ...
                  <div className="w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center font-display font-black text-2xl"
                    style={{ background: `hsl(${i * 140 % 360}, 50%, 15%)`, color: '#00FF66', border: '1px solid #00FF66' }}>
                    {c.name[0]}
                  </div>
                  <p className="font-semibold text-base mb-1" style={{ color: '#F5F0E8' }}>{c.name}</p>
                  <p className="text-xs font-bold mb-0.5" style={{ color: '#00FF66' }}>{c.followers}</p>
                  <p className="text-xs" style={{ color: 'rgba(245, 240, 232, 0.4)' }}>{c.platform}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── EXAMPLE VIDEOS ── */}
      <section id="videos" className="py-28 px-6 md:px-16" style={{ background: '#1f1f1f' }}>
        <div className="max-w-7xl mx-auto">
          <ScrollReveal>
            <h2 className="font-display font-black text-5xl md:text-6xl mb-16 text-center" style={{ color: '#F5F0E8' }}>
              Example Videos
            </h2>
          </ScrollReveal>
            ...
            {videos.map((v, i) => (
              <ScrollReveal key={v.title} delay={i * 100}>
                <div className="card-glow rounded-2xl overflow-hidden group flex flex-col bg-transparent h-full"
                     style={{ border: '1px solid rgba(0,255,102,0.1)' }}>
                  <div className="relative w-full aspect-[9/16] bg-black">
                    <video src={v.videoUrl} className="w-full h-full object-cover" autoPlay muted loop playsInline />
                    <span className="absolute top-3 left-3 px-2 py-1 rounded-md text-xs font-semibold"
                      style={{ background: 'rgba(0,255,102,0.9)', color: '#000', zIndex: 10 }}>
                      {v.tag}
                    </span>
                    <span className="absolute top-3 right-3 text-[10px] font-bold uppercase tracking-wider"
                      style={{ color: '#00FF66', zIndex: 10, textShadow: '0px 1px 4px rgba(0,0,0,0.8)' }}>
                      {v.views}
                    </span>
                  </div>
                  <div className="py-4 px-4">
                    <p className="text-sm font-medium leading-snug" style={{ color: '#F5F0E8' }}>{v.title}</p>
                  </div>
                </div>
              </ScrollReveal>
            ))}
 ...
 {/* ── REFERRALS ── */}
      <section id="referrals" className="py-28 px-6 md:px-16" style={{ background: '#050505' }}>
        <div className="max-w-7xl mx-auto">
          <ScrollReveal>
            <p className="text-xs font-semibold tracking-widest uppercase mb-4" style={{ color: '#00FF66' }}>Testimonials</p>
            <h2 className="font-display font-black text-5xl md:text-6xl mb-16" style={{ color: '#fff' }}>
              Referrals
            </h2>
          </ScrollReveal>
                ...
                <div className="card-glow rounded-2xl p-8 h-full flex flex-col"
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm"
                      style={{ background: '#00FF66', color: '#fff' }}>
                      {r.name[0]}
                    </div>
      ...
      {/* ── CTA BANNER ── */}
      <div style={{ background: '#050505', paddingBottom:'90px' }}>
        <section className="py-24 px-6 mx-6 md:mx-16 rounded-3xl text-center relative overflow-hidden"
          style={{ background: 'linear-gradient(135deg, #050505 0%, #001a0a 100%)', border: '1px solid rgba(0,255,102,0.2)' }}>
          <div className="relative z-10">
            <h2 className="font-display font-black text-4xl md:text-6xl mb-6" style={{ color: '#F5F0E8' }}>
              Ready to go viral?
            </h2>
            <Link href="/apply"
              className="inline-flex items-center gap-2 px-10 py-4 rounded-full text-base font-bold transition-all hover:scale-105"
              style={{ background: '#00FF66', color: '#000', boxShadow: '0 8px 32px rgba(0,255,102,0.3)' }}>
              Apply Now →
            </Link>
          </div>
        </section>
